...
class RepositoryPrivacyTest(unittest.TestCase):
    def test_common_provider_secret_files_are_ignored(self):
        ...
        self.assertTrue(
            {".env", ".env.*", "*.env", "doubao.env", "credentials*.json", "secrets*.json", "*.key", "*.pem", ".opentutor/", "*.log"}
            .issubset(rules)
        )
    ...
    def test_public_schema_ids_use_the_cassian_atlas_namespace(self):
        root = Path(__file__).resolve().parents[1]
        for path in (root / "schemas").glob("*.json"):
            schema = json.loads(path.read_text(encoding="utf-8"))
            self.assertTrue(
                schema["$id"].startswith("https://example.invalid/cassian-atlas/"),
                path.name,
            )
