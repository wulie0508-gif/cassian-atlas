...
class AgentOrchestrationTest(unittest.TestCase):
    ...
    def test_router_builds_minimal_specialist_pipelines(self):
        ...
        image_answers = plan_route(
            "请用 Codex 和豆包从手写答题图片转写翻译答案，并让我批次确认后入库。",
            student_id="STU-001",
        )
        self.assertEqual(
            [row["capability_key"] for row in image_answers["steps"]],
            ["evidence-confirmation"],
        )
        self.assertEqual(image_answers["primary_capability"], "evidence-confirmation")

        base_projection = plan_route(
            "把 STU-001 的运营周报暂存为飞书 Base 投影，不要更新本地学习事实。",
            student_id="STU-001",
        )
        self.assertEqual(
            [row["capability_key"] for row in base_projection["steps"]],
            ["base-projection"],
        )
        self.assertEqual(
            [row["skill"] for row in base_projection["steps"]],
            ["$publish-learning-projection"],
        )

        local_dashboard = plan_route(
            "刷新本地 Agent 工作流的最近运行状态。",
            student_id="STU-001",
        )
        self.assertEqual(
            [row["capability_key"] for row in local_dashboard["steps"]],
            ["dashboard-sync"],
        )
