...
class PublicShowcaseContractTests(unittest.TestCase):
    ...
    def test_public_assets_are_local_and_scripts_are_not_inline(self):
        ...
        self.assertEqual(script_sources, ["app.js?v=0.5.0"])
        self.assertNotRegex(self.html, r"<script(?![^>]+src=)[^>]*>")
        self.assertIn('href="styles.css?v=0.5.0"', self.html)
    ...
    def test_private_corpus_offer_is_explicit_and_kept_outside_mit_bundle(self):
        self.assertIn("PRIVATE CORPUS", self.html)
        self.assertIn('href="mailto:wulie0508@gmail.com"', self.html)
        self.assertIn("公开仓库不包含试卷、文章、答案、学生记录或 API 凭据", self.html)
        self.assertIn("以书面资料清单和授权约定为准", self.html)
