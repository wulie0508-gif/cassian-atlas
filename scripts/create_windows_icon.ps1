param(
  [string]$Output = (Join-Path $PSScriptRoot '..\docs\assets\opentutor-ledger.ico')
)
...
public static class OpenTutorNativeIcon {
...
try {
  ...
  $stream.Dispose()
  $icon.Dispose()
  [OpenTutorNativeIcon]::DestroyIcon($handle) | Out-Null
  $cream.Dispose()
  $mint.Dispose()
  $graphics.Dispose()
  $bitmap.Dispose()
