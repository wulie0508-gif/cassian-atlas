    ...
    *,
    ...
    if before["status"] in {"unmanaged", "incompatible", "unhealthy"}:
        raise RuntimeError(
            ...
            "Inspect `opentutor server status`; stop the old process before starting this version."
        )
    ...
    while time.monotonic() < deadline:
        if process.poll() is not None:
            state_file.unlink(missing_ok=True)
            raise RuntimeError(
                f"OpenTutor server exited during startup with code {process.returncode}; "
                f"inspect {log_path}"
            )
    ...
    try:
        ...
        process.kill()
    state_file.unlink(missing_ok=True)
    raise RuntimeError(f"OpenTutor server did not become healthy within {timeout:g} seconds")
...
def stop_server(
    ...
    *,
    ...
    while time.monotonic() < deadline and _pid_alive(pid):
        time.sleep(0.2)
    if _pid_alive(pid):
        raise RuntimeError(f"OpenTutor server PID {pid} did not stop within {timeout:g} seconds")
    state_file.unlink(missing_ok=True)
    return {
        "status": "stopped",
        "result": "stopped",
