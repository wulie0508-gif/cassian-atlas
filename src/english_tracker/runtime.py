...
def _validate_raw(raw: object) -> dict[str, str]:
    if not isinstance(raw, dict):
        raise ValueError("Cassian Atlas config must be a JSON object")
    unknown = sorted(set(raw) - set(CONFIG_KEYS) - {"config_version"})
    if unknown:
        raise ValueError(f"Unknown Cassian Atlas config key(s): {', '.join(unknown)}")
    version = raw.get("config_version", CONFIG_VERSION)
    if version != CONFIG_VERSION:
        raise ValueError(
            f"Unsupported Cassian Atlas config_version {version!r}; expected {CONFIG_VERSION}"
        )
    values: dict[str, str] = {}
    for key in CONFIG_KEYS:
        ...
        if not isinstance(value, str) or not value.strip():
            raise ValueError(f"Cassian Atlas config '{key}' must be a non-empty string")
    ...
    if db_name and (
        ...
        raise ValueError("Cassian Atlas config 'db_name' must be a plain SQLite filename")
    return values
...
def load_runtime_config(
    ...
    *,
    ...
    if not path.exists():
        ...
        raise ValueError(f"Cassian Atlas config not found: {path}")
    try:
        raw = json.loads(path.read_text(encoding="utf-8-sig"))
    except json.JSONDecodeError as exc:
        raise ValueError(f"Invalid Cassian Atlas config JSON at {path}: {exc}") from exc
...
def set_config_value(path: Path, key: str, value: str) -> RuntimeConfig:
    if key not in CONFIG_KEYS:
        raise ValueError(f"Unknown Cassian Atlas config key: {key}")
    value = str(value).strip()
    if not value:
        raise ValueError("Cassian Atlas config values cannot be empty")
    raw: dict[str, object] = {}
    if path.exists():
        try:
