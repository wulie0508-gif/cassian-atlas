...
from .contracts import (
    validate_attempt_replacements_payload,
    ...
    *,
    ...
    conn.execute(
        ...
        INSERT OR IGNORE INTO ingest_event_rows(
          ingest_event_id, entity_type, entity_id, action, before_json, after_json
        ) VALUES (?, ?, ?, ?, ?, ?)
        """,
        (
            event_id,
...
def import_attempts(
    ...
    *,
    ...
    """Import one idempotent attempt envelope.

    Normal callers leave ``manage_transaction`` enabled.  A higher-level
    audited workflow may disable it only while already holding the owning
    transaction; this lets that workflow commit its gate and the resulting
    learning facts atomically.
    """
...
def replace_session_attempts(
    conn,
    payload: dict,
    *,
    actor: str = "engineering",
    reason: str = "audited attempt replacement",
    backup_path: str | None = None,
) -> dict[str, Any]:
    """Atomically replace selected active attempts and refresh their assessment.

    This operation is intentionally narrower than a whole-event correction.  It
    preserves the original session and item identities, supersedes only the
    explicitly named active attempts, then derives review scheduling and the
    session assessment from the complete active attempt set in one transaction.
    """
    payload = validate_attempt_replacements_payload(payload)
    ingest_event_id = payload["event_id"]
    actor = str(actor or "engineering").strip()
    reason = str(reason or "audited attempt replacement").strip()
    if not actor:
        raise IngestConflict("actor is required")
    if not reason:
        raise IngestConflict("reason is required")

    with conn:
        created, existing = _begin_event(
            conn,
            payload,
            "attempt_replacement",
            backup_path,
        ...
        assessment = conn.execute(
            "SELECT * FROM session_assessments WHERE session_id=?",
            (session_id,),
        ).fetchone()
        if not assessment:
            raise IngestConflict("The target session has no session_assessment to recalculate")
        baseline = conn.execute(
            """
            SELECT
              COUNT(*) AS attempt_count,
              COUNT(DISTINCT a.item_id) AS distinct_item_count,
              COUNT(e.evaluation_id) AS evaluation_count,
              SUM(CASE WHEN e.score IS NULL OR e.max_score IS NULL THEN 1 ELSE 0 END)
                AS missing_score_count,
              SUM(e.score) AS raw_score,
              SUM(e.max_score) AS max_score,
              SUM(CASE WHEN a.answer_capture_status='captured_blank' THEN 1 ELSE 0 END)
                AS blank_count
            FROM attempts a
            LEFT JOIN evaluations e ON e.attempt_id=a.attempt_id AND e.is_current=1
            WHERE a.session_id=? AND a.student_id=? AND a.record_status='active'
            """,
            (session_id, student_id),
        ).fetchone()
        if (
            not baseline
            or not baseline["attempt_count"]
            or baseline["attempt_count"] != baseline["distinct_item_count"]
            or baseline["attempt_count"] != baseline["evaluation_count"]
            or baseline["missing_score_count"]
        ):
            raise IngestConflict(
                "The existing assessment does not have one fully scored active attempt per item"
            )
        if (
            assessment["raw_score"] is None
            or assessment["max_score"] is None
            or assessment["blank_count"] is None
            or abs(float(assessment["raw_score"]) - float(baseline["raw_score"])) > 1e-9
            or abs(float(assessment["max_score"]) - float(baseline["max_score"])) > 1e-9
            or int(assessment["blank_count"]) != int(baseline["blank_count"] or 0)
        ):
            raise IngestConflict(
                "session_assessment does not match the current active-attempt baseline; "
        ...
        for replacement in payload["replacements"]:
            old = conn.execute(
                ...
                SELECT
                  a.*,
                  e.evaluation_id AS current_evaluation_id,
                  e.result AS current_result,
                  e.score AS current_score,
                  e.max_score AS current_max_score,
                  e.evaluated_by AS current_evaluated_by,
                  e.is_human_corrected AS current_is_human_corrected,
                  e.note AS current_evaluation_note
                FROM attempts a
                JOIN evaluations e ON e.attempt_id=a.attempt_id AND e.is_current=1
                WHERE a.attempt_id=? AND a.student_id=? AND a.session_id=?
                  AND a.record_status='active'
                """,
                (replacement["old_attempt_id"], student_id, session_id),
            ).fetchone()
            if not old:
                raise IngestConflict(
                    "Unknown active attempt for the selected learner/session: "
                    f"{replacement['old_attempt_id']}"
                )
            old_max_score = old["current_max_score"]
            new_max_score = float(replacement["evaluation"]["max_score"])
            if old_max_score is None or float(old_max_score) != new_max_score:
                raise IngestConflict(
                    "Attempt replacement cannot change the scored slot max_score: "
                    f"{replacement['old_attempt_id']}"
                )
            new_attempt_id = str(
                replacement.get("attempt_id") or stable_id("ATT", replacement["event_id"])
            ).strip()
            if new_attempt_id in new_attempt_ids:
                raise IngestConflict(f"Duplicate replacement attempt_id: {new_attempt_id}")
            new_attempt_ids.add(new_attempt_id)
            collision = conn.execute(
                "SELECT attempt_id FROM attempts WHERE attempt_id=? OR event_id=?",
                (new_attempt_id, replacement["event_id"]),
            ).fetchone()
            if collision:
                raise IngestConflict(
                    "Replacement attempt identity already exists outside this idempotent event: "
                    f"{collision['attempt_id']}"
                )
            targets.append((dict(old), replacement, new_attempt_id))

        now = utc_now()
        affected_items: set[str] = set()
        replacement_results: list[dict[str, Any]] = []
        for old, replacement, new_attempt_id in targets:
            affected_items.add(old["item_id"])
            evaluation = replacement["evaluation"]
            standard_answer = (
                replacement["standard_answer"]
                if "standard_answer" in replacement
                else old["standard_answer_snapshot"]
            )
            attempted_at = replacement.get("attempted_at") or old["attempted_at"]
            teacher_note = (
                replacement["teacher_note"]
                if "teacher_note" in replacement
                else old["teacher_note"]
            )
            source_material_ref = (
                replacement["source_material_ref"]
                if "source_material_ref" in replacement
                else old["source_material_ref"]
            )
            conn.execute(
                """
                INSERT INTO attempts(
                  attempt_id,event_id,ingest_event_id,student_id,session_id,item_id,
                  artifact_id,attempted_at,student_answer,standard_answer_snapshot,
                  answer_capture_status,attempt_phase,response_mode,validation_status,
                  teacher_note,source_material_ref,supersedes_attempt_id,created_at
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                """,
                (
                    new_attempt_id,
                    ...
                    old["artifact_id"],
                    attempted_at,
                    replacement.get("student_answer"),
                    standard_answer,
                    replacement["answer_capture_status"],
                    replacement.get("attempt_phase") or old["attempt_phase"],
                    replacement.get("response_mode") or old["response_mode"],
                    replacement.get("validation_status") or old["validation_status"],
                    teacher_note,
                    source_material_ref,
                    old["attempt_id"],
                    now,
            ...
            new_attempt = dict(
                conn.execute("SELECT * FROM attempts WHERE attempt_id=?", (new_attempt_id,)).fetchone()
            )
            _record_event_row(
                conn,
                ingest_event_id,
            ...
            evaluation_id = stable_id("EVAL", new_attempt_id, 1)
            conn.execute(
                """
                INSERT INTO evaluations(
                ...
                (
                    ...
                    evaluation.get("evaluated_by", "teacher"),
                    int(bool(evaluation.get("is_human_corrected", True))),
                    evaluation.get("note"),
                    now,
                ),
            )
            ...
            for error in replacement.get("error_types", []):
                error_type_id, raw, confidence, note = _resolve_error_type(conn, error)
                error_key = (error_type_id, raw)
                if error_key in seen_errors:
                    raise IngestConflict(
                        f"Duplicate replacement error mapping for attempt {new_attempt_id}: {raw}"
                    )
                ...
                mapping = dict(
                    conn.execute(
                        ...
                        WHERE attempt_id=? AND error_type_id=? AND raw_error_type IS ?
                        """,
                        (new_attempt_id, error_type_id, raw),
                    ).fetchone()
                )
                mapping_id = stable_id("DIAG", new_attempt_id, error_type_id, raw)
                _record_event_row(
                    conn,
                    ingest_event_id,
                    "attempt_error_map",
            ...
            old_errors = list(
                conn.execute(
                    """
                    SELECT rowid AS mapping_rowid,* FROM attempt_error_map
                    WHERE attempt_id=? AND record_status='active'
                    """,
                    (old["attempt_id"],),
                )
            )
            for old_error_row in old_errors:
                old_error = dict(old_error_row)
                conn.execute(
                    """
                    UPDATE attempt_error_map
                    SET record_status='voided', invalidation_reason=?
                    WHERE rowid=?
                    """,
                    (
                        f"Parent attempt superseded by {new_attempt_id} in {ingest_event_id}.",
                        old_error["mapping_rowid"],
                    ),
                )
                after_error = dict(
                    conn.execute(
                        "SELECT rowid AS mapping_rowid,* FROM attempt_error_map WHERE rowid=?",
                        (old_error["mapping_rowid"],),
                    ).fetchone()
                )
                mapping_id = stable_id(
                    "DIAG",
                    old["attempt_id"],
                    old_error["error_type_id"],
                    old_error["raw_error_type"],
                )
                _record_event_row(
                    conn,
                    ingest_event_id,
            ...
            conn.execute(
                "UPDATE attempts SET record_status='superseded' WHERE attempt_id=?",
                (old["attempt_id"],),
            )
            after_old_attempt = dict(
                conn.execute(
            ...
            conn.execute(
                ...
                (
                    ...
                    canonical_json(
                        {
                            "attempt": before_old_attempt,
                            "evaluation": {
                                "evaluation_id": old["current_evaluation_id"],
                                "result": old["current_result"],
                                "score": old["current_score"],
                                "max_score": old["current_max_score"],
                            },
                        }
                    ),
                    canonical_json(
                        {
                            "superseded_attempt": after_old_attempt,
                            "replacement_attempt": new_attempt,
        ...
        review_tasks: dict[str, str | None] = {}
        for item_id in sorted(affected_items):
            review_tasks[item_id] = _rebuild_review_tracking(
                conn,
                student_id,
                item_id,
                ingest_event_id,
        ...
        if not aggregate or not aggregate["attempt_count"]:
            raise IngestConflict("The corrected session has no active attempts")
        if aggregate["distinct_item_count"] != aggregate["attempt_count"]:
            raise IngestConflict(
                "Assessment recalculation requires exactly one active scored attempt per item in the session"
            )
        if aggregate["evaluation_count"] != aggregate["attempt_count"]:
            raise IngestConflict("Every active session attempt must have one current evaluation")
        if aggregate["missing_score_count"]:
            raise IngestConflict(
                "Every active session evaluation must have numeric score and max_score before assessment recalculation"
            )
        raw_score = float(aggregate["raw_score"])
        max_score = float(aggregate["max_score"])
        blank_count = int(aggregate["blank_count"] or 0)
        if max_score <= 0 or not 0 <= raw_score <= max_score:
            raise IngestConflict("Recalculated assessment scores are outside valid bounds")

        before_assessment = dict(assessment)
        conn.execute(
            """
            UPDATE session_assessments
            SET raw_score=?,max_score=?,blank_count=?
            WHERE session_id=?
            """,
            (raw_score, max_score, blank_count, session_id),
        )
        after_assessment = dict(
            conn.execute(
                "SELECT * FROM session_assessments WHERE session_id=?",
                (session_id,),
            ).fetchone()
        ...
        conn.execute(
            ...
            (
                ...
                session_id,
                ingest_event_id,
                canonical_json(before_assessment),
                canonical_json(after_assessment),
                reason,
            ),
        )
        _finish_event(
            conn,
            ingest_event_id,
            len(replacement_results) + 1,
            len(replacement_results) + 1,
        )
        mark_completed_generations_stale(conn, student_id=student_id)
    ...
    return {
        ...
        "open_review_tasks": sum(task_id is not None for task_id in review_tasks.values()),
        "assessment": {
            "raw_score": raw_score,
            "max_score": max_score,
            "blank_count": blank_count,
            "active_attempt_count": int(aggregate["attempt_count"]),
        },
    }
...
def undo_ingest_event(conn, target_event_id: str, *, actor: str = "engineering", reason: str = "correction") -> dict[str, Any]:
    with conn:
        ...
        if event["event_type"] == "attempt_replacement":
            raise IngestConflict(
                "Atomic attempt-replacement events cannot be reverted with generic ingest undo; "
                "replace the current active attempt again or restore the verified pre-write backup."
            )
        affected = [
...
def _rebuild_review_state(
    ...
    item_id: str,
    *,
    refresh_manual_evidence: bool = False,
) -> None:
    current = conn.execute(
        "SELECT * FROM review_state WHERE student_id=? AND item_id=?",
        (student_id, item_id),
    ).fetchone()
    if current and current["manual_override"] and not refresh_manual_evidence:
        return
    rows = list(
        conn.execute(
            """
    ...
    if current and current["manual_override"]:
        conn.execute(
            """
            UPDATE review_state
            SET repetitions=?,lapses=?,consecutive_errors=?,last_attempt_id=?,
...
def _rebuild_review_tracking(
    conn,
    student_id: str,
    item_id: str,
    ingest_event_id: str,
) -> str | None:
    """Rebuild derived state and the single actionable task for one item."""
    before_state_row = conn.execute(
        "SELECT * FROM review_state WHERE student_id=? AND item_id=?",
        (student_id, item_id),
    ).fetchone()
    before_state = dict(before_state_row) if before_state_row else None
    _rebuild_review_state(
        conn,
        student_id,
        item_id,
        refresh_manual_evidence=True,
    )
    ...
    for task_row in list(
        ...
        conn.execute(
            "UPDATE review_tasks SET status='voided' WHERE review_task_id=?",
            (task_row["review_task_id"],),
        )
        after_task = dict(
            conn.execute(
    ...
    if (
        not latest
        or latest["result"] == "correct"
        or (state is not None and state["state"] == "suspended")
    ):
        return None

    due_at = (
        (state.get("due_at") if state else None)
        or (_parse_time(latest["attempted_at"]) + timedelta(days=1)).isoformat()
    )
    consecutive = int(state.get("consecutive_errors") or 0) if state else 1
    task_id = stable_id("RT", student_id, item_id, latest["attempt_id"], ingest_event_id)
    conn.execute(
        """
        INSERT INTO review_tasks(
          review_task_id,student_id,item_id,source_attempt_id,reason_code,
          due_at,priority,status,created_by_event_id,created_at
        ) VALUES (?,?,?,?,?,?,?,'open',?,?)
        """,
        (
            task_id,
            ...
            item_id,
            latest["attempt_id"],
            "needs_check" if latest["result"] == "needs_check" else "incorrect_attempt",
            due_at,
            1.0 + min(consecutive, 5) * 0.2,
            ingest_event_id,
            now,
