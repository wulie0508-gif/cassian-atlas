...
CAPABILITIES = (
    Capability(
        ...
        responsibility="以 cassian CLI 为控制平面维护安装、配置、迁移、启动和多学生工作区；看板仅作只读状态投影。",
        mode="engineering_write",
        endpoints=("cassian", "/api/health", "/api/agent/dashboard"),
    ),
    Capability(
    ...
    Capability(
        ...
        skill="confirm-learning-evidence",
        name_cn="作答提取与人工确认",
        name_en="Evidence extraction confirmation",
        responsibility="将图片、OCR和模型转写保留为候选，覆盖全部题目完成人工批次确认后才事务化写入学习事实。",
        mode="review_then_write",
        endpoints=(
            "cassian extraction",
            "/api/extraction/batches",
            "/api/extraction/batches/{batch_id}/review",
            "/api/extraction/batches/{batch_id}/commit",
        ),
    ),
    Capability(
    ...
    Capability(
        ...
        responsibility="从到期队列和薄弱点出发，以只读题库快照生成已核验、去重、完整语篇且不可变的选题清单；旧语法目录继续支持 weighted set-cover。",
        mode="verified_manifest_write",
        endpoints=(
            "cassian selection",
    ...
    Capability(
        key="base-projection",
        skill="publish-learning-projection",
        name_cn="学习运营投影",
        name_en="Learning operations projection",
        responsibility="校验 Cassian Learning Lab 目标并把聚合运营指标暂存到本地 Base outbox；在线飞书写入必须另行授权、核验身份并完成精确回读。",
        mode="local_stage_then_approved_publish",
        endpoints=("cassian projection",),
    ),
    Capability(
...
_SIGNALS: dict[str, tuple[str, ...]] = {
    ...
    "evidence-confirmation": (
        "手写", "作答图片", "答题图片", "答题卡", "答案照片", "图片提取", "模型提取",
        "转写", "豆包", "多模型", "双模型", "人工确认", "批次确认", "图片识别", "ocr",
        "transcription", "answer image", "confirmation batch",
    ),
    ...
    "base-projection": (
        "飞书 base", "飞书多维表格", "base 投影", "base同步", "base 同步",
        "运营投影", "家长看板同步", "发布到飞书", "同步到飞书", "feishu base",
        "projection outbox",
    ),
...
def capability_manifest() -> dict[str, Any]:
    return {
        ...
        "architecture": {
            ...
            "control_plane": "The cassian CLI is the control plane; the dashboard is a read-only projection for status and evidence.",
...
def plan_route(request_text: str, *, student_id: str, subject_code: str = "english") -> dict[str, Any]:
    ...
    elif scores["evidence-confirmation"]:
        selected.append("evidence-confirmation")
        if scores["mistake-diagnosis"]:
            selected.append("mistake-diagnosis")
    elif scores["base-projection"]:
        selected.append("base-projection")
